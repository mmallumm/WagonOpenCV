...
Wagon::Wagon(cv::Mat background, double first_side, double second_side) :
    ...
    for(int i = 0; i < point_number; ++i) {
        ...
        node.point.y = trajectory_amplitude*sin(trajectory_step * i / trajectory_period) + background.rows/2;
        node.angle = 0;
    ...
    for(int i = 0; i < point_number - 1; ++i) {
        ...
        double cos_alpha = vector_x/sqrt(vector_x*vector_x + vector_y*vector_y);
        trajectory_[i].angle = (vector_y >= 0) ? degree(acos(cos_alpha)) : -degree(acos(cos_alpha));
    }
}
...
void Wagon::drive_1() {
    ...
    for(int i = 0; i < trajectory_.size(); ++i) {
        ...
        set_position(trajectory_[i].point.x, trajectory_[i].point.y, trajectory_[i].angle);
        draw_wagon(current_background);
        if(i == trajectory_.size()/2) {
            cv::imwrite("../save_1.jpg", current_background);
        }
...
void Wagon::drive_2() {
    ...
    for(int i = 0; i < trajectory_.size(); ++i) {
        ...
        draw_wagon(current_background);
        if(i != 0) {
            cv::line(background, trajectory_[i-1].point, trajectory_[i].point, violet, 5);
        }
        if(i == trajectory_.size()/2) {
            cv::imwrite("../save_2.jpg", current_background);
        }        
        cv::imshow("lab1", current_background);
        cv::waitKey(25);
    }
...
void Wagon::draw_wagon(cv::Mat background) {
    ...
    cv::Point2d p0(-half_a * cos(radian(angle_)) - -half_b * sin(radian(angle_)) + x_, 
                   -half_a * sin(radian(angle_)) + -half_b * cos(radian(angle_)) + y_);
    cv::Point2d p1(half_a * cos(radian(angle_)) - -half_b * sin(radian(angle_)) + x_, 
                   half_a * sin(radian(angle_)) + -half_b * cos(radian(angle_)) + y_);  
    cv::Point2d p2(half_a * cos(radian(angle_)) - half_b * sin(radian(angle_)) + x_, 
                   half_a * sin(radian(angle_)) + half_b * cos(radian(angle_)) + y_);
    cv::Point2d p3(-half_a * cos(radian(angle_)) - half_b * sin(radian(angle_)) + x_, 
                   -half_a * sin(radian(angle_)) + half_b * cos(radian(angle_)) + y_);         
...
void Wagon::draw_full_trajectory(cv::Mat background) {
    ...
    for(int i = 0; i < trajectory_.size() - 1; ++i) {
        cv::line(background, trajectory_[i].point, trajectory_[i+1].point, violet, 5);
    }
}
